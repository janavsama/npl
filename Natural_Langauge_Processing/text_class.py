...
class Textastic:
    ...
    @staticmethod
    def _load_stop_words(words):
        """ Removes stop words from a list of words"""
        stopwords = {"0o", "0s", "3a", "3b", "3d", "6b", "6o", "a", "a1", "a2", "a3", "a4", "ab", "able", "about",
                     ...
                     "you've", "yr", "ys", "yt", "z", "zero", "zi", "zz", ",", "-", "__", "--", "4"}

        return [word for word in words if word.lower() not in stopwords]

    def load_text(self, filename, label=None, parser=None):
        """Loads text from a file, processes it using a parser function,
    ...
    def _parse_data(self, k=10, user_defined_words=None):
        """Parses the loaded text data to extract word counts and sentiment scores."""
        word_counts = defaultdict(Counter)
        sentiment_scores = defaultdict(dict)

        for text_name, data in self.data.items():
            words = data['word_count']
            sentiment = data['sentiment_scores']
            if user_defined_words:
                words = {word: count for word, count in words.items() if word in user_defined_words}
            else:
                words = dict(sorted(words.items(), key=lambda item: item[1], reverse=True)[:k])
            word_counts[text_name] = words
            sentiment_scores[text_name] = sentiment

        nodes = list(self.data.keys())
        for text_name, words in word_counts.items():
            nodes.extend(words.keys())
            nodes.extend(sentiment_scores[text_name].keys())

        return nodes, word_counts, sentiment_scores

    def _create_sankey(self, nodes, word_counts):
        """ Creates a Sankey diagram visualization for the text-to-word relationship."""
        sources = []
        targets = []
        values = []

        for i, (text_name, words) in enumerate(word_counts.items()):
            for word, count in words.items():
                sources.append(i)
                targets.append(nodes.index(word))
                values.append(count)
    ...
    def wordcount_sankey(self, k=10, user_defined_words=None):
        """Generates a Sankey diagram to visualize word counts in the loaded text data."""
        nodes, word_counts, sentiment_scores = self._parse_data(k, user_defined_words)
        self._create_sankey(nodes, word_counts)

    def generate_wordcloud(self):
        """Generates a word cloud visualization for each loaded text."""
        for text_name, data in self.data.items():
            words = data['word_count']
            ...
            plt.title(f"Word Cloud for {text_name}")
            plt.axis('off')
            plt.show()

    def generate_word_frequency_plot(self):
        """Generates a bar plot showing the frequency of the top words in each loaded text."""
        for text_name, data in self.data.items():
            words = data['word_count']
            sorted_words = dict(sorted(words.items(), key=lambda item: item[1], reverse=True))

            words = list(sorted_words.keys())
            frequencies = list(sorted_words.values())

            plt.figure(figsize=(10, 6))
            plt.barh(words[:10], frequencies[:10], color='olive')
            plt.xlabel('Frequency')
            plt.ylabel('Words')
            plt.title(f'Top 10 Words Frequency in {text_name}')
